...
async def train_model(file: UploadFile = File(...)):
    ...
    for name, model in models.items():
        ...
        if name == "Linear Regression":
            y_pared = [1 if pared > 0.5 else 0 for pared in y_pared]
...
async def predict(file: UploadFile = File(...)):
    if not trained:
        return {"error": "Model not trained yet. Use /train first."}

    test_df = pd.read_csv(file.file)
    x_test_data = test_df.drop('target', axis=1)
    predictions = {}
    for name, model in models.items():
        y_pared = model.predict(x_test_data)
        predictions[name] = y_pared.tolist()
